...
async def talk_with_avatar(message: ApiUserMessage):
    ...
    if redis and cached_response and len(cached_response)>0:
        ...
        def cached_stream_generator(cached_data):
            ...
            response, blendshapes, visemes, audio = pickle.loads(cached_data[b'response']), pickle.loads(cached_data[b'blendshapes']), pickle.loads(cached_data[b'visemes']), pickle.loads(cached_data[b'audio'])
        ...
        def stream_generator(response) -> Tuple[Any, str]:
            ...
            for phrase in TTS.text_preprocessor(response, filter=None, use_ssml=True):
                logger.debug(f"TIMER: text chunk recieved @ {datetime.now().timestamp()*1000}")
                ...
                redis.append_to_cache(cache_key,'response',full_text)  # Add the full text of the response for usin in conversation memory.
                
                logger.debug(f"YEILDED:(B,V):: {len(blendshapes), len(visemes)}")
                logger.debug(f"TIMER: Yeilded audio chunk @ {datetime.now().timestamp()*1000}")
