...
class YAMLMenuHandler(BaseMenuHandler):
    ...
    Arguments:
        ...
        menu_filename (str, *optional*): name of the menu file.
    ...
    Usage:
        Single Menu file: If there is only a single yaml file at path then it is used as the menu source file. Specifying a menu_filename will override this behaviour
        Selecting menu form MenuList: If the menu_filename is specified and the file contains a list of Menus then a sinlge menu is selected according to the
    ...
    def get_menu(self) -> Menu:
        ...
        if self.path is None:
            raise RuntimeError(f"a path must specified when sourcing menus from disk.")
        ...
        if menu_filename is None:
            ...
            if len(filelist) > 1:
                raise RuntimeError(
                    f"menu folder for {self.cafe_id} contains more than 1 yaml file. Please pass in menu_filename"
                )
            menu_filename = filelist[0]  # If there's only one yaml file then use it.

        if "." not in menu_filename:
            menu_filename += ".yaml"

        if menu_filename in filelist:
            raw_menus = OmegaConf.load(
                os.path.join(self.path, self.cafe_id, menu_filename)
            )
            if hasattr(raw_menus, "menus"):
                ...
                if self.name is not None:
                    ...
                    if selected_menu is None:
                        raise RuntimeError(
                            f"Menu names {self.name} not found in menu list saved as {menu_filename}"
                        )
                    ...
                    raise RuntimeError(
                        f"One of menu name or use_time_of_day must to set in order to select a menu"
                    )
            else:  # its a single menu file
                selected_menu = Menu.from_omega_conf(
                    OmegaConf.load(os.path.join(self.path, self.cafe_id, menu_filename))
                )
        else:
            raise RuntimeError(
                f"{menu_filename} not found in {os.path(self.path, self.cafe_id)}"
            )
