...
class AudioConnection(ABC):
    ...
    def __init__(
        self, conn, audio_settings={}, audio_processing_func: Callable = None, **kwargs
    ):
...
class WebRTCAudioSteam(AudioConnection):
    ...
    def __init__(
        self,
        conn: webrtc_streamer = None,
        audio_settings: dict = {},
        audio_processing_func: Callable = None,
        ...
        **kwargs,
        ...
        if conn is None:
            self.conn, self.audio_settings = setup_webRTC(
                use_ice_server=("local" in kwargs), timeout_sec=timeout_sec, **kwargs
            )  # This is blocking
    ...
    def default_processor(self, data: av.AudioFrame):
        ...
        data = np.ascontiguousarray(
            data.to_ndarray()[0][0 :: self.audio_settings["num_channels"]]
        )  # Allows for audiochannels to be interleaved
        ...
        if self.ready():
            try:
                ...
                while len(self.frames) < self.audio_settings["required_frames"]:
                    self.frames.extend(
                        self.conn.audio_receiver.get_frames(timeout=timeout)
                    )  # call to webRTC audio_reciever (airtc)
            except queue.Empty:
                logger.warning(
                    f"Audio queue failed to receive a input from the microphone within {timeout} seconds. This could be due to network congestion, audio device or driver problems."
                )
...
def setup_webRTC(
    use_ice_server: str = False,
    ice_servers: List[str] = ["stun:stun.l.google.com:19302"],
    timeout_sec: float = 60.0,
    st_status_bar: Any = None,
    **kwargs,
) -> (webrtc_streamer, Dict):
    """
    ...
    if use_ice_server:
        ...
        webrtc_ctx = webrtc_streamer(
            ...
            rtc_configuration={"iceServers": [{"urls": ice_servers}]},
    ...
    for i in tqdm(
        range(timeout_sec)
    ):  # note this gets interrupted each time there is a postback event.
    ...
    if WEBRTC_CONNX_ESTABLISHED in st.session_state:
        ...
        audio_settings = {
            "required_frames": math.ceil(
                float(FRAMES_PER_BUFFER / first_packet.samples)
            ),  # min frames required for AssemblyAi API. A good choice is 4800 samples
            "sample_rate": first_packet.sample_rate,  # sample rate of incoming sound
            "num_channels": len(
                first_packet.layout.channels
            ),  # stereo or mono. AssemblyAI requires mono.
        }
        st.session_state[
            WEBRTC_CONNX_ESTABLISHED
        ] = True  # Flag that the settings have been collected.
        st.session_state[WEBRTC_CONNECTION] = (
            webrtc_ctx,
            audio_settings,
        )  # persist the connection and setting through post-back
        return webrtc_ctx, audio_settings
    else:
        # return placeholder object
        st_status_bar.write(
            f"Failed to connect to server within {timeout_sec} seconds. Please refresh page to try again."
        )
        return None, None
