...
WEBRTC_CONNECTION = (
    "connx"  # persitance of the connection and audio data from the streamer.
)
...
async def send_receive(args, audio_stream: AudioConnection):
    ...
    async with websockets.connect(
        ...
        session_begins = (
            await _ws.recv()
        )  # defer until the connection to assembly ai is established.
        ...
        async def receive(args):
            ...
            system_prompt = (
                None  # Sytem prompt to be used to instruct Lllama2 how to respond
            )
                    ...
                    if (
                        ...
                        if sys_keywords in json.loads(result_str)["text"].lower():
                            system_content = re.sub(
                                sys_keywords, "", json.loads(result_str)["text"]
                            ).strip()
                            if len(system_content) > 10:
                                system_prompt = InferencePrompt(
                                    role="system", content=system_content
                                )
                            else:
                                st_status_bar.write(
                                    "! system prompt input was too short so it was ignored."
                                )
                            ...
                            if system_prompt is not None:
                                ...
                                prompt = [
                                    ...
                                    InferencePrompt(
                                        role="user",
                                        content=json.loads(result_str)["text"],
                                    ),
                                ]
                            else:
                                prompt = [
                            ...
                            payload = InferenceSessionPrompt(
                                prompt=prompt, session_id=st.session_state[SESSION_KEY]
                            )
                            st.session_state[
                                "conversation"
                            ] += f"\n\nYou: \n {prompt[-1].content}"
                            ...
                            async with aiohttp.ClientSession() as session:  # TODO make this one clientsession per instance not per request for latency reasons.
                                async with session.post(
                                    url=endpoint(args.endpoint_base_URL, args.task),
                                    json=payload.json(),
                                ) as r:
                                    try:
                                        data = await r.json()
                                        ...
                                        logger.error(
                                            f"Key error. Return message from endpoint: {data}"
                                        )
                                    except Exception as e:
        ...
        send_result, receive_result = await asyncio.gather(
            send(args, audio_stream), receive(args)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="The task name which is alsow the endpoint name.",
    )
    parser.add_argument(
        "--system_keywords",
        type=str,
        default="system prompt",
        help="phrase used to start setting of system prompt",
    )
    parser.add_argument(
        "--tokenizer_model_path",
        type=str,
        default="./tokenizer.model",
        help="used to calculate the tokens in the conversation history",
    )
    parser.add_argument(
        "--mode",
        type=str,
        choices=["quiet", "debug"],
        default="quiet",
        help="debug mode exposes results from STT API call",
    )
    parser.add_argument(
        "--local",
        "-l",
        action="store_true",
        default=False,
        help="Set this flag if no ICE server is needed.",
    )
    parser.add_argument(
        "--stream_type",
        type=str,
        choices=["web", "local"],
        default="local",
        help="Wether audio is sources from a browser or local machine.",
    )
    ...
    try:
        llm_params = json.loads(
            requests.get(endpoint(args.base, "endpoint_base_URL")).content
        )
        logger.info(f"Max sequence length:{llm_params['max_seq_len']}")
        args.max_history_length = llm_params[
            "max_seq_len"
        ]  # makes sure that max context window in LLM is used in managing the length of the chat_history.
    except Exception as e:
        logger.info(
            "/llm_params endpoint in found. Default arguments from app will be used for LLM parameters."
        )

    # create the audio source connection.
    if args.stream_type == "web":
        audio_stream = WebRTCAudioSteam(
            None, local=args.local, timeout_sec=60, st_status_bar=st_status_bar
        )
    elif args.stream_type == "local":
        audio_stream = PyAudioStream(
            None, st_status_bar=st_status_bar
        )  # Default are in the class - TODO refactor to config file.

    # Chat history window.
    conversation_txt = st.text_area(
        "**Chat Window**",
        st.session_state["conversation"],
        key="conversation_txt",
        height=500,
    )
