...
def create_agent(args)-> str:
	if 'session_id' not in st.session_state:
		payload = SessionStart(cafe_id = args.cafe_id,
								rule_names = {
		...
		response = requests.get(
			url=f'{args.llm_endpoint.strip("/")}/create_agent_session', json=payload.dict()
		)
			...
			while "run" in st.session_state and st.session_state["run"]:
				try:
					...
					if (
						...
						user_input = ApiUserMessage( user_input= f"{json.loads(result_str)['text']}",
